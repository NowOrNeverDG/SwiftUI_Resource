        ...
        HStack(spacing: kImageSpace) {
            ForEach(images,id: \.self) { image in
                loadImage(name: image)
                    .resizable()
                    .scaledToFill()
                    .frame(width: (self.width - kImageSpace*CGFloat(self.images.count - 1)) / CGFloat(self.images.count), height: (self.width - 6 * CGFloat(self.images.count - 1)) / CGFloat(self.images.count))
                    .clipped()
            }
        }
    }
...
struct PostImageCell_Previews: PreviewProvider {
    static var previews: some View {
        ...
        .previewLayout(.fixed(width: width, height: 300))//指定预览页的宽高
    }
}
